...
if {[catch { vlog -incr -sv \
    ...
    +define+TARGET_FLASH_VIP \
    +define+TARGET_I2C_VIP \
    +define+TARGET_I2S_VIP \
    +define+TARGET_RTL \
    +define+TARGET_RTL_SIM \
    +define+TARGET_RT_DPI \
    +define+TARGET_SIMULATION \
    +define+TARGET_TEST \
    +define+TARGET_USE_VIPS \
    +define+TARGET_VSIM \
...
if {[catch { vlog -incr -sv \
    ...
    +define+TARGET_USE_VIPS \
    +define+TARGET_VSIM \
    "$ROOT/target/sim/vip/spi_flash/S25fs256s/model/s25fs256s.v" \
}]} {return 1}
...
if {[catch { vlog -incr -sv \
    ...
    +define+TARGET_USE_VIPS \
    +define+TARGET_VSIM \
    "$ROOT/target/sim/vip/i2c_eeprom/24FC1025.v" \
}]} {return 1}
...
if {[catch { vlog -incr -sv \
    ...
    +define+TARGET_USE_VIPS \
    +define+TARGET_VSIM \
    "$ROOT/target/sim/vip/i2s/i2c_if.v" \
    "$ROOT/target/sim/vip/i2s/i2s_vip_channel.sv" \
    "$ROOT/target/sim/vip/i2s/i2s_vip.sv" \
}]} {return 1}
